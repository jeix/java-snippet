...
public class WrapperOverCollection_2_JSON {
	...
	public record BizRuleGroup(String code, String name, List<BizRuleType> types) {
		...
		public String toJson() {
			var typesJson = types.stream().map(BizRuleType::toJson)
				.collect(java.util.stream.Collectors.joining(","));
			var sb = new StringBuilder();
			sb.append("{\"code\":\"").append(code).append("\",");
			sb.append("\"name\":\"").append(name).append("\",");
			sb.append("\"types\":[").append(typesJson).append("]");
			sb.append("}");
			return sb.toString();
		}
	}
	...
	public record BizRuleSet(
		...
		public String toJson() {
			var majorsJson = majors.stream().map(BizRuleType::toJson)
				.collect(java.util.stream.Collectors.joining(","));
			var recentsJson = recents.stream().map(BizRuleType::toJson)
				.collect(java.util.stream.Collectors.joining(","));
			var othersJson = others.stream().map(BizRuleGroup::toJson)
				.collect(java.util.stream.Collectors.joining(","));

			var sb = new StringBuilder();
			sb.append("{\"majors\":[").append(majorsJson).append("],");
			sb.append("\"recents\":[").append(recentsJson).append("],");
			sb.append("\"others\":[").append(othersJson).append("]}");
			return sb.toString();
		}
	}
	...
	public static void main(String[] args) {
		...
		System.out.println(overview.ruleset().toJson());
	}
}